import type { GitHubUser, GitHubRepository, GitHubSearchResponse } from "./types"
...
export async function searchUsers(query: string): Promise<GitHubUser[]> {
  ...
  try {
    const data = await fetchFromGitHub<GitHubSearchResponse>(`/search/users?q=${encodeURIComponent(query)}&per_page=5`)
    return data.items
  } catch (error) {
